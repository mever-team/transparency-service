...
def pipeline(data):
    ...
    images = torch.stack(images).to(device) # To GPU # shape = batch_size, 3, width, hight
    with torch.no_grad():
...
metrics = evaluation.run(data = data_path, pipeline = pipeline, task = 'Image Classification', batch_size = 32)
print(metrics)
