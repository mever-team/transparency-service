...
def custom_tokenizer(data):
    ...
    return tokenizer.encode_plus(input_str, max_length=512, padding="max_length", truncation=True, return_tensors="pt")
...
class customodel(torch.nn.Module):
    ...
    def forward(self, input_ids):
        ...
        return torch.stack([logit[:, 0] for logit in logits])  # logit[:, 1] 'Fake' logit[:,0] 'Real'

device = 'cuda'
model = customodel()
mc = transparency_service.model_card_generator.ModelCard()
transparency_service.evaluation.std.evaluate(model, custom_tokenizer, device = device, model_card = mc)
