...
class _Github:
	def git_get_license(self, owner: str, repo: str):
		...
		self.json['Model Details']['License'] = f"{license_info.license.spdx_id if license_info else 'No license info'}"		
	...
	def get_git_info(self, owner: str, repo: str):
		...
		self.json['Model Details']['Version'] = f"{tags[0].name if tags.totalCount > 0 else 'No releases found'}"
