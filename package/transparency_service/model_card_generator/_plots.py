...
class _Plots:
		# base 64
	def bar_plot(self, pdata: pd.DataFrame, title: str, x: str, y: str, ptype :str = 'ipython',
				 xaxis_title: str = '', yaxis_title: str ='', width: int = 700, height: int = 700, color = None,
				 barmode="relative"):
		if ptype == 'ipython':
	...
	def insert_img(self,
				   file_path: str,
				   destination: str, title: str
				   ):
		self.json[destination][title] = file_path
	# Bar plots	
	def fill_bar_plot(self,
					  plot_title: str,
					  labels: List[str],
					  values: List[float]
					  ):
	...
	def init_bar_plot(self,
		...
		if title in self.bar_plot_data:
			...
			return
		self.bar_plot_data[title] = {
			"bars": [],
			"xlabel": xlabel,
			"ylabel": ylabel
		}	
		
	def append_bar_to_plot(self,
						   title: str,
						   label: str,
						   value: float):
		if title not in self.bar_plot_data:
			print(f"Plot with title '{title}' does not exist. Create it first using 'init_bar_plot'.")
			return
		self.bar_plot_data[title]["bars"].append((label, value))
		
	def add_plot_to(self,
					title: str,
					destination: str
					): # TODO: Generaliez this
		if title not in self.bar_plot_data:
	...
	def add_interactive_plot_to(self,
								title: str,
								destination: str,
		...
		if shap_plot != -1:
			...
			def on_close(event):
				...
				if base64encode:
					...
					src = f"{destination.replace(' ', '_')}_{title.replace(' ', '_')}.png"
			...
			print(f"Plot with title '{title}' does not exist, and no SHAP plot was provided.")
			return
		else:
