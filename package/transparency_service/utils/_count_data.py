...
def write_md(model_card, data_count, is_train_set):
    ...
    if isinstance(data_count[next(iter(data_count))], dict): # we have a split
        data_count_pd = pd.DataFrame.from_dict(data_count, orient='columns').reset_index()
        data_count_pd.columns = ['split'] + list(data_count.keys())
        data_count_pd = data_count_pd.melt(id_vars = ['split'], value_vars = data_count.keys(),
                                           var_name='set', value_name='count')
        model_card.text['Training Set' if is_train_set else 'Eval Set']['Train Set plot' if is_train_set else 'Eval Set plot'] = (
            model_card.bar_plot(
                pdata=data_count_pd,
                y='count',
                x='split',
                ...
                yaxis_title='Number of Data').replace('<div>', '<div class="plot-inline-div">'))
    else:
    ...
    for p in path:
        ...
        if not is_file(p): # folder
            ...
            if 'Files' in data: # folder -> files
                for file_path in data['Files']:
                    if determine_type(file_path) in get_supported_images(): # if Images
                        c = len(data['Files'])
                        break
                    current = count_data(file_path, split, model_card = None) # if not Images
            ...
            if split is not None:
                ...
                for chunk in data:
                    for key in chunk:
                        count[os.path.splitext(os.path.basename(p))[0]][key] += len(chunk[key])
