...
def _unknown_columns_handler(data, number_of_samples, length_of_sample):
    ...
    for column_name in data.column_names:
        ...
        if len(my_set) <= 20:
            out[f'values of {column_name} and number of instances'] = [f"{s}: {len(data.filter(lambda x: str(x[column_name]) == s))}" for s in my_set]
            ...
            my_set = set(items)
            out[f'values of {column_name} and number of instances'] = [f"{s}: {sum(1 for label in items if label == s)}" for s in my_set]
        else:
            out[f'sample for the first {number_of_samples} rows of {column_name}'] = [f'{data[column_name][i]}'[:length_of_sample] for i in range(number_of_samples)]
    return out
...
def _extract_data_info(
    ...
    out = out_in
    out['column_names'] = data.column_names if len(anns.column_names) == 0 else data.column_names + anns.column_names
