...
def print_aligned_table(rows):
    ...
    out = ''
    # Print headers
    out += f"{'-' * col_widths[0]}----"f"{'-' * col_widths[1]}----"f"{'-' * col_widths[2]}"
    out += f"{headers[0]:>{col_widths[0]}}    "f"{headers[1]:>{col_widths[1]}}    "f"{headers[2]:>{col_widths[2]}}"
    out += f"{'=' * col_widths[0]}===="f"{'=' * col_widths[1]}====" f"{'=' * col_widths[2]}"

    # Print row
    for row in rows:
        out += f"{row[0]:>{col_widths[0]}}    "f"{row[1]:>{col_widths[1]}}    "f"{row[2]:>{col_widths[2]}}"
    out += f"{'=' * col_widths[0]}===="f"{'=' * col_widths[1]}===="f"{'=' * col_widths[2]}"
...
def model_summary(model: "Pytorch or Tensorflow", input_data=None):
    ...
    raise TypeError("Unknown model type or missing dependencies. Make sure it's a PyTorch or TensorFlow model.")
