...
def check_and_download_data():
    try: # try 7z
        process = subprocess.Popen(["7z", "--help"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    except FileNotFoundError:  # No 7z case
        print("Please install 7-Zip. Make sure you can run the 7z command in your terminal.")
        return False
    ...
    if not os.path.exists(data_dir):
        ...
        for f in files:
            ...
            downloaddata('https://github.com/several27/FakeNewsCorpus/archive/refs/tags/v1.0.zip',
                     f"{data_dir}/FakeNewsCorpus/FakeNewsCorpus-1.0.zip")
            downloaddata('https://github.com/several27/FakeNewsCorpus/archive/refs/tags/v1.0.tar.gz',
        ...
        downloaddata("https://github.com/rpitrust/fakenewsdata1/raw/refs/heads/master/Horne2017_FakeNewsData.zip",
        ...
        downloaddata('https://raw.githubusercontent.com/thiagorainmaker77/liar_dataset/master/test.tsv',
        ...
        for ds in [
            ...
            for split in ['valid', 'test']:  # 'train',
                ...
                downloaddata(f"https://openaipublic.azureedge.net/gpt-2/output-dataset/v1/{filename}",
...
def prepare_data():
    ...
    try: # try 7z
        ...
        reliable_file = os.path.expanduser(f"{dir}/news_cleaned_2018_02_13.reliable.csv")
        fake_file = os.path.expanduser(f"{dir}/news_cleaned_2018_02_13.fake.csv")
        with open(input_file, mode='r', encoding='utf-8') as infile, \
            open(reliable_file, mode='w', encoding='utf-8', newline='') as reliable_out, \
            open(fake_file, mode='w', encoding='utf-8', newline='') as fake_out:
    ...
    # Extract
    with zipfile.ZipFile(os.path.join(dir, 'Horne2017_FakeNewsData.zip'), 'r') as zip_ref:
        zip_ref.extractall(dir)
    # Read data and conver to csv
    rootfolder = f"{dir}/Public Data"
    folders = {
        "Fake": [f"{rootfolder}/Buzzfeed Political News Dataset/Fake",
                 f"{rootfolder}/Random Poltical News Dataset/Fake"],
        "Fake_titles": [f"{rootfolder}/Buzzfeed Political News Dataset/Fake_titles",
                        f"{rootfolder}/Random Poltical News Dataset/Fake_titles"],
        "Real": [f"{rootfolder}/Buzzfeed Political News Dataset/Real",
                 f"{rootfolder}/Random Poltical News Dataset/Real"],
        "Real_titles": [f"{rootfolder}/Buzzfeed Political News Dataset/Real_titles",
    ...
    def read_files(text_folders, title_folders, label):
        for text_folder, title_folder in zip(text_folders, title_folders):
            ...
            for text_file, title_file in zip(text_files, title_files):
                ...
                try:
                    with open(text_path, 'r', encoding='cp1252', errors='ignore') as tf, \
                            open(title_path, 'r', encoding='cp1252', errors='ignore') as tt:
                        text = tf.read().strip()
                        title = tt.read().strip()
...
def get_data(set: str, tags: List[str]) -> pd.DataFrame:
    ...
    if set == 'FakeNewsCorpus':
        ...
        for t in tags:
            if t == 'title':
                out_pd['title'] = pd.concat([fake_news['title'], reliable_news['title']], ignore_index=True)
            elif t == 'content':
                out_pd['content'] = pd.concat([fake_news['content'], reliable_news['content']], ignore_index=True)
            elif t == 'label':
                out_pd['label'] = pd.concat([fake_news['type'], reliable_news['type']], ignore_index=True)
            else:
        ...
        validation_pd['account.type'] = validation_pd['account.type'].replace({'human': 0, 'bot': 1})
        out_pd = pd.DataFrame()
        for t in tags:
            if t == 'title':
...
class EvaluationDataset(Dataset):
    def __init__(self, data_path, tags, tokenizer):
        ...
        self.data = get_data(data_path, tags)  # Assumes CSV has 'title' and 'content' columns
...
def write_md(model_card, eval_pd):
    ...
    model_card.json['Quantitative Analysis']['Text'] = f"""Those plots were generated by the transparency service module.
...
def evaluate(model, tokenizer, device = None, tuple_id = None, batch_size=64, num_workers=12,
             model_card: ModelCard = None):
    check_and_download_data()
    if device is None:
        device = 'cuda' if torch.cuda.is_available() else 'cpu'

    eval_pd = run_evaluation(model = model, tokenizer = tokenizer, device = device, tuple_id = tuple_id,
