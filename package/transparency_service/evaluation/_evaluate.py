...
def read_data_pd(path, delimiter = None, names = None, split = None):
    ...
    if t == supported_types[0]:  # .csv
        ...
        data = pd.read_csv(path, names = names, engine="python") if delimiter is None else pd.read_csv(path, delimiter=delimiter, names = names, engine="python")
...
def calc_metrics(
    ...
    if 'class' in task.lower(): # all classification cases
        ...
        if isinstance(target[0], int):
            ...
            if num_classes < 2:
                raise ValueError(f"found only {num_classes} classes in the dataset. Can't calculate metrics")
            elif num_classes == 2:
                class_task = 'binary'
            else:
        ...
        for key in data.column_names: # search for the images as path or web
            if is_path(data[key][0][0]):
                ...
                image_formats = ["image/"+t.replace('.','') for t in get_supported_image_types()]
            ...
            for batch in data:
                for img, bboxes in zip(batch[img_column], batch[bbox_column]):
                    ...
                    for bbox in bboxes:
                        ...
                        if box_format is not None: break
                    if box_format is not None: break
                if box_format is not None: break
        elif src_type == 'url':
        ...
        if not isinstance(preds[0], dict):
            for pred in preds:
                ...
                preds_ready.append({"boxes": torch.tensor(boxes).to(device), "labels": torch.tensor(cat_ids).to(device), "scores": torch.tensor(scores).to(device)})
        else:
            preds_ready = preds
            ...
            for batch in anns_source:
                for boxes_target, cat_ids_target in zip(batch[bbox_column], batch[label_column]):
                    if (boxes_target is not None) and (cat_ids_target is not None): # prevent reading None values that the convertion to datasets creates
                        target_ready.append({"boxes": torch.tensor(boxes_target).to(device), "labels": torch.tensor(cat_ids_target).to(device)})
        else:
            # obj, bbox, label = target
            if isinstance(anns_source[0][obj_column], dict):
                for batch in anns_source:
                    for boxes_target, cat_ids_target in zip(batch[obj_column][bbox_column], batch[obj_column][label_column]):
                        if (boxes_target is not None) and (cat_ids_target is not None): # prevent reading None values that the convertion to datasets creates
                            target_ready.append({"boxes": torch.tensor(boxes_target).to(device), "labels": torch.tensor(cat_ids_target).to(device)})
            else: # elif isinstance(data[obj], list)
                for batch in anns_source:
                    for object in batch[obj_column]:
                        if (object[bbox_column] is not None) and (object[label_column] is not None): # prevent reading None values that the convertion to datasets creates
                            target_ready.append({"boxes": torch.tensor(object[bbox_column]).to(device), "labels": torch.tensor(object[label_column]).to(device)})
...
def tasks2instance():
    return {
        ...
        'Image Classification': ([torch.Tensor, int, float, List[float], str, dict[str, float]], lambda x: (isinstance(x, (torch.Tensor, int, float, str)) or (isinstance(x, list) and all(isinstance(i, float) for i in x)) or (isinstance(x, dict) and all(isinstance(k, str) and isinstance(v, float) for k, v in x.items())))),
        'Object Detection': ([dict[str, torch.Tensor],List[List[int]], List[int], List[float]], lambda x: (isinstance(x, list) and len(x) == 3 and isinstance(x[0], list) and all(isinstance(sublist, list) and len(sublist) == 4 and all(isinstance(i, int) for i in sublist) for sublist in x[0]) and isinstance(x[1], list) and all(isinstance(i, int) for i in x[1]) and isinstance(x[2], list) and all(isinstance(f, float) for f in x[2])) or (isinstance(x, dict) and 'scores' in x and isinstance(x['scores'], torch.Tensor) and 'labels' in x and isinstance(x['labels'], torch.Tensor) and 'boxes' in x and isinstance(x['boxes'], torch.Tensor))), # box, cat_id, score
        ...
        'Image - to - Video': (List[np.ndarray], lambda x: isinstance(x, list) and all(isinstance(t, np.ndarray) for t in x)),
        'Video Classification': ([torch.Tensor, int, float, List[float], str, dict[str, float]], lambda x: (isinstance(x, (torch.Tensor, int, float, str)) or (isinstance(x, list) and all(isinstance(i, float) for i in x)) or (isinstance(x, dict) and all(isinstance(k, str) and isinstance(v, float) for k, v in x.items())))),
        'Text - to - Video': (List[np.ndarray], lambda x: isinstance(x, list) and all(isinstance(t, np.ndarray) for t in x)),
        ...
        'Keypoint Detection': (List[Tuple[int, int]], lambda x: isinstance(x, list) and all(isinstance(t, tuple) and len(t) == 2 and isinstance(t[0], int) and isinstance(t[1], int) for t in x)),
        # Natural Language Processing
        'Text Classification': ([torch.Tensor, int, float, List[float], str, dict[str, float]], lambda x: (isinstance(x, (torch.Tensor, int, float, str)) or (isinstance(x, list) and all(isinstance(i, float) for i in x)) or (isinstance(x, dict) and all(isinstance(k, str) and isinstance(v, float) for k, v in x.items())))),
...
def tasks2targets_object_special_case():
    seg_targets = {'target': ['seg'], 'label': ['cat', 'label'], # candidate names for target. Must be a set of those
                   'obj': ['objects']} # candidate names for object
    objdetect_targets = {'target': ['box'],'label':  ['cat', 'label'],
                         'obj': ['objects']}
    return {
...
def tasks2metrics():
    return {
        # Multimodal
        'Audio - Text - to - Text': ['wer', 'cer', 'blue', 'rouge'], # Word Error Rate (WER), Character Error Rate (CER), blue, rouge
        'Image - Text - to - Text': ['blue', 'rouge', 'meteor', 'cider', 'spice'],
        'Visual Question Answering': ['EM', 'blue', 'meteor', 'VQA Accuracy'], # Exact Match (EM), f1, blue, meteor, VQA Accuracy # 'f1',
        'Document Question Answering': ['EM', 'blue', 'rouge', 'meteor'], # 'f1',
        'Video - Text - to - Text': ['blue', 'rouge', 'meteor', 'CIDEr', 'spice'],
        # Computer Vision
        'Depth Estimation': ['mae', 'rmse', 'ssim', 'sirmse'],# Mean Absolute Error (mae), Root Mean Square Error (rmse), Structural Similarity Index (ssim), Scale-Invariant rmse
        'Image Segmentation': ['IoU', 'Pixel acc',  'map', 'dice'],
        'Object Detection': ['map', 'IoU'], # Mean Average precision (map), Intersection over Union (IoU), recall, precision, f1 # , 'f1'
        'Image Classification': ['precision', 'recall', 'f1', 'acc', 'roc-auc'],
        'Text - to - Image': ['IS', 'fid', 'CLIPScore', 'ssim'],# Inception Score (IS), Fréchet Inception Distance (fid), CLIPScore, Structural Similarity Index (ssim)
        'Image - to - Text': ['blue', 'rouge', 'meteor', 'CIDEr', 'spice'],
        'Image - to - Image': ['ssim', 'psnr', 'lpips'],# Structural Similarity Index (ssim), Peak Signal-to-Noise Ratio (psnr), lpips (Perceptual Loss)
        'Image - to - Video': ['FVD' , 'ssim', 'psnr'], # FVD (Fréchet Video Distance), ssim, psnr
        'Video Classification': ['precision', 'recall', 'f1', 'acc', 'roc-auc'],
        'Text - to - Video': [ 'FVD', 'IS', 'fid', 'ssim'],
        'Mask Generation': ['IoU', 'Pixel Accuracy', 'dice'],
        'Image Feature Extraction': ['Cosine Similarity', 'Euclidean Distance', 'lpips'],
...
def check_validity_of_output(out_sample, task):
    ...
    if task in t2i:
        ...
        if isok:
            ...
            raise ValueError(f'Expected one of {", ".join(str(i) for i in t2i[task][0])}, but got {repr(out_sample)}')
    else:
        raise ValueError(f'''Task of type "{task}", is not supported. Choose one of {', '.join('"'+i+'"' for i in t2i)}''')
...
def check_validity_of_target(data, task, target_column):
    ...
    if len(targets_found) == 0:
        raise ValueError(f'''Expected one of {', '.join('"'+i+'"' for i in t2t)}, but got {', '.join('"'+key+'"' for key in data)}''')
    elif len(targets_found) > 1:
        raise ValueError(f'''Found more that one match to target: {', '.join('"'+i+'"' for i in targets_found)}. Use the option "target" to choose which one you want''')
    else:
...
def run(
    ...
    target_column = check_validity_of_target(data[0] if len(anns.features) == 0 else anns[0], task, target_column)
