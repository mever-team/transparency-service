...
class AI_Assistant:
    ...
    def _inject_text_with_hints(self, text: "str", tips: "dict"):
        ...
        pattern = re.compile(
            "|".join(re.escape(key) for key in tips.keys()), re.IGNORECASE
        )
        ...
        def replacement_function(match):
            ...
            key = next(
                key for key in tips.keys() if key.lower() == matched_text.lower()
            )
            value = tips[key]
    ...
    def simplify(self, text: "str"):
        ...
        messages = [
            {
                "role": "system",
                "content": """You are an AI specialized in simplifying technical texts while maintaining a professional, academic tone. Your goal is to rewrite technical content in a way that preserves all information but replaces complex terminology with clear, accessible language.
							Instructions:
        ...
        simplified = self.__run_model(
            messages, max_tokens=4000, temperature=0.7, top_p=1
        )
        return simplified

    def dataset_summary(
        self,
        dataset: "path to csv, json, tsv, parquet and more or dict, list, datasets.Dataset",
        anns=[None],
    ):
    ...
    def suggest_ethical_considerations(
        self,
    ...
    def __split_pdf_to_chunks(
        self, pdf_path: str, max_characters: int = 10000  # path to pdf
    ) -> List[str]:
            ...
            for page in reader.pages:
                ...
                for line in text.splitlines():
                    if len(current_chunk) + len(line) > max_characters:
                        ...
                        if last_period != -1:
                            self.pdf_chunks_for_llm.append(
                                current_chunk[: last_period + 1]
                            )
                            current_chunk = (
                                current_chunk[last_period + 1 :].strip() + line + "\n"
                            )
                        else:
    ...
    def create_openai_json(self, pdf_path: str, model_card=None) -> str:  # Path to pdf
        ...
        system_prompt = importlib.resources.read_text(
            "transparency_service.ai_assistant", "role_for_improve_json_with_openai.txt"
        )
        ...
        for i, chunk in enumerate(self.pdf_chunks_for_llm):
            ...
            model_output = self.__run_model(
                messages, max_tokens=4000, temperature=0.000001, top_p=0.000001
            )
    ...
    def create_overview(
        ...
        system_prompt = importlib.resources.read_text(
            "transparency_service.ai_assistant", "summ.ai"
        )
