...
class Prompter(Assistant):
    ...
    def _complete(self, text: str, task: str, card: ModelCard, logger: Logger, user_messages: list[str]):
        ...
        if self.agent._model == 'Inference/Schematron:3b':
            output_formats = card.json_schema()
            prompt = text
            params = {"format": output_formats, "temperature": 0}

            # Extra parameterization

            output_formats['$defs']['model']['properties']['overview']['minLength'] = 500
            output_formats['$defs']['model']['required'] = ['name', 'overview', 'author']
            output_formats['$defs']['considerations']['required'] = ['use_case', 'users']
            for cat, fields in output_formats['$defs'].items():
                if not isinstance(fields, dict): continue
                if 'more' in output_formats['$defs'][cat]['properties']:
                    output_formats['$defs'][cat]['properties'].pop('more')
                for field, values in output_formats['$defs'][cat]['properties'].items():
                    output_formats['$defs'][cat]['properties'][field].pop('description')
            # output_formats['$defs']['performance']['required'] = ['analysis']
            ...
            for category, values in card.data.items():
                ...
                for field, value in values.items():
                    ...
                    new_value = completion[category][field]
                    if not new_value: continue
