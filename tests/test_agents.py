...
def test_import_metrics(client, admin_token):
    # 1. create card
    ...
    response = client.post("/transparency/card", json={"title": "pytest_import_metrics"}, headers={"Authorization": f"Bearer {admin_token}"})
    assert response.status_code == 201
    card_id = int(response.data.decode())
