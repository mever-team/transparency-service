...
def ai_data_summary(file_path, session_id):
    ...
    ai_data_summ = globals[session_id].aia.dataset_summary(data_pd).replace('\n', '<br>')
...
def construct_metrics_table_html(metrics, session_id):
    ...
    globals[session_id].metrics_table = '<div style="overflow-x: auto;"><table id="dataTable" border="0"><thead><tr>'
    ...
    for line in metrics:
        ...
        for column_name in metrics.column_names:
            try:
                globals[session_id].metrics_table += f'<td style="text-align: center;">{round(line[column_name], 4)}</td>'
            except (ValueError, TypeError):
                globals[session_id].metrics_table += f'<td style="text-align: center;">{line[column_name]}</td>'
        globals[session_id].metrics_table += '</tr>'
...
def construct_terms_tips_table(session_id):
    ...
    for term, tip in globals[session_id].mc.tips.items():
        ...
        globals[session_id].terms_tips_table += f'<textarea id="term_{term}">{term}</textarea>'
        globals[session_id].terms_tips_table += f'<textarea id="tip_{tip}">{tip}</textarea>'
        globals[session_id].terms_tips_table += f'<button class="dynamic-btn" data-button-name="save_{term}">Save</button>'
        globals[session_id].terms_tips_table += f'<button class="dynamic-btn" data-button-name="remove_{term}">Remove</button>'
...
def construct_model_details_textareas(session_id):
    ...
    for key, text in globals[session_id].mc.text["Model Details"].items():
        ...
        globals[session_id].model_details_textareas += f'<textarea id="{key}">{text}</textarea>'
...
def construct_considerations_fields(session_id):
    ...
    for key, text in globals[session_id].mc.text["Considerations"].items():
        ...
        globals[session_id].considerations_fields += f'<textarea id="{key}">{text}</textarea>'
...
def level_of_details_editor(args, session_id):
    if args.button == "Add AI tips":
        for field in globals[session_id].mc.text:
            if field != "Title":
                for text in globals[session_id].mc.text[field].values():
                    globals[session_id].mc.tips = globals[session_id].mc.tips | globals[session_id].aia._create_hints(text)
            ...
            for field in globals[session_id].mc.text:
                if field != "Title":
                    for key in globals[session_id].mc.text[field]:
                        print(f"old globals[session_id].mcwithtips.text[{field}][{key}]" + globals[session_id].mcwithtips.text[field][key])
                        globals[session_id].mcwithtips.text[field][key] = globals[session_id].aia._inject_text_with_hints(globals[session_id].mc.text[field][key], globals[session_id].mc.tips)
                        print(f"new globals[session_id].mcwithtips.text[{field}][{key}]" + globals[session_id].mcwithtips.text[field][key])

        for field in globals[session_id].mc.text:
            if field != "Title":
                for key in globals[session_id].mc.text[field]:
                    print(f"old globals[session_id].mcsimplified.text[{field}][{key}]" + globals[session_id].mcwithtips.text[field][key])
                    globals[session_id].mcsimplified.text[field][key] = globals[session_id].aia.simplify(globals[session_id].mc.text[field][key])
                    print(f"old globals[session_id].mcsimplified.text[{field}][{key}]" + globals[session_id].mcwithtips.text[field][key])
        ...
        globals[session_id].mcwithtips.save(f'.{session_templates_path}/~model_card_with_tip')
...
def model_details_editor(args, session_id):
    if args.button == "Submit from model details":
        ...
        globals[session_id].mc.text['Model Details']['Overview'] += "<br>******** This is AI generated start<br>" + globals[session_id].aia.create_overview(file_path).replace('\n', '<br>') + "<br>******** This is AI generated stop<br>"
    construct_model_details_textareas(session_id)

def considerations_editor(args, session_id):
    if args.button == "Submit from aichat":
        globals[session_id].suggest_ethical_considerations_chat_list.append({"role": "user", "content": args.user_input})
        ethical_suggestions = globals[session_id].aia.suggest_ethical_considerations(full_message_list = globals[session_id].suggest_ethical_considerations_chat_list)
        globals[session_id].suggest_ethical_considerations_chat_list.append({"role": "assistant", "content": ethical_suggestions})
        globals[session_id].airesponse = ''
        for message in globals[session_id].suggest_ethical_considerations_chat_list:
            globals[session_id].airesponse = globals[session_id].airesponse + '\n' + message['content'] + '\n******************************************'
...
def training_set_editor(args, session_id):
    if args.button == "Submit from training set":
        ...
        split_data = [item.split(': ') for item in globals[session_id].data_extracted_info_train[args.button]]
        data_to_plot_pd = pd.DataFrame(split_data, columns=[plot_name, 'count'])
        data_to_plot_pd['count'] = pd.to_numeric(data_to_plot_pd['count'])

        globals[session_id].mc.plots['Training Set'][plot_name] = globals[session_id].mc.bar_plot(pdata=data_to_plot_pd, y='count', x=plot_name, title=plot_name, yaxis_title='Number of instances')
...
def eval_set_editor(args, session_id):
    if args.button == "Submit from eval set":
        ...
        split_data = [item.split(': ') for item in globals[session_id].data_extracted_info_eval[args.button]]
        data_to_plot_pd = pd.DataFrame(split_data, columns=[plot_name, 'count'])
        data_to_plot_pd['count'] = pd.to_numeric(data_to_plot_pd['count'])

        globals[session_id].mc.plots['Eval Set'][plot_name] = globals[session_id].mc.bar_plot(pdata=data_to_plot_pd, y='count', x=plot_name, title=plot_name, yaxis_title='Number of instances')


def quantitative_analysis_editor(args, session_id):
    if args.button == "Submit from quantitative analysis":
        globals[session_id].mc.text['Quantitative Analysis']["Description"] = args.textarea["quantitative_analysis"].replace('<figure class="table"', '<figure class="table" style="overflow-x: auto;"')
        ...
        globals[session_id].mc.text['Quantitative Analysis']["Description"] = globals[session_id].aia.metrics_summary(file_path).replace('\n', '<br>') + '<br>' + globals[session_id].metrics_table
